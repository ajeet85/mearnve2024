...
export default function Benefit({ gktab }) {
  ...
  useEffect(() => {
    const singleContent = contentList.find((ele) => ele.menu === 'benifit');
      ...
      <section className="tabs-section">  
        <div className="container">  
          <ul className="nav nav-tabs">
            <li className={gktab === '1' ? 'active' : ''}><Link to="?gktab=1" className="buyer-tab-icon">Buyer</Link></li>
            <li className={gktab === '2' ? 'active' : ''}><Link to="?gktab=2" className="supplier-tab-icon">Supplier</Link></li>
            <li className={gktab === '3' ? 'active' : ''}><Link to="?gktab=3" className="funder-tab-icon">Funder</Link></li>
            <li className={gktab === '4' ? 'active' : ''}><Link to="?gktab=4" className="introducer-tab-icon">Introducer</Link></li>
          </ul>
        </div>
        <div className="tab-content">
          ...
          <div id="tab-four" className={`tab-pane fade ${gktab === '4' ? 'in active' : ''}`}>
                  {postData && (
                        <>
                        <div dangerouslySetInnerHTML={{ __html: removeExtraSpaces(postData.content_section4) }} />
